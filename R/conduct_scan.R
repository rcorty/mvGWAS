...
mvGWAS$methods(
    ...
    scan_vcf_file_ = function(file_name,
        ...
        for (snp_idx in 1:num_snps) {
            ...
            alt_fit <- tryNULL(dglm::dglm2(formula = metadata$mean_alt_formula,
            ...
            if (exists(x = 'mean_null_formula', where = metadata)) {
                mean_null_fit <- tryNULL(dglm::dglm2(formula = mean_null_formula,
            ...
            if (exists(x = 'var_null_formula', where = metadata)) {
                var_null_fit <- tryNULL(dglm::dglm2(formula = mean_alt_formula,
            ...
            if (all(exists(x = 'metadata$mean_null_formula', where = metadata),
                ...
                joint_null_fit <- tryNULL(dglm::dglm2(formula = mean_null_formula,
                                                      dformula = var_null_formula,
                                                      data = this_locus_df,
                                                      method = 'ml'))
...
mvGWAS$methods(
    ...
    conduct_scan = function(mean_formula,
        ...
        null_model <<- dglm::dglm2(formula = metadata$mean_null_formula,
